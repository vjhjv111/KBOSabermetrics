...
모바일은 타격할 위치를 터치하면 바로 그곳으로 스윙합니다. PC는 마우스/방향키 조준과 클릭/SPACE를 지원하며, 투수는 기존 드래그 조준과 투구 버튼을 사용합니다. 팀별 의상·이름과 장비를 표현하는 공용 3D 선수 모델을 사용하며 확인되지 않은 등번호는 표시하지 않습니다.
