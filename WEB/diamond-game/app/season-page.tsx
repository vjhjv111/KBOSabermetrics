...
type Tab='game'|'league'|'career'|'practice'|'friendly';
export default function SeasonPage(){
 const [tab,setTab]=useState<Tab>(()=>new URLSearchParams(location.search).has('room')?'friendly':new URLSearchParams(location.search).has('match')?'practice':'game');
 ...
 const client=useSeasonClient(visible&&tab==='game'&&!savePanelOpen),{state,career,roster,busy,loading,error,command}=client;
 ...
 const save=state?.save,game=save?.game,teams=roster?.teams??defaultTeams;
 ...
 const send=(op:string,extra:Record<string,unknown>={})=>void command({op,...extra}).catch(()=>{});
 ...
 return <main className="season-app" style={{'--club-color':accent} as React.CSSProperties}>
  ...
  {tab==='practice'?<Practice/>:<div className="season-content">
   ...
   {loading?<div className="season-loading"><RefreshCw size={28}/><h2>구단 기록과 저장된 시즌을 불러옵니다</h2><p>처음에는 선수 데이터를 준비하는 데 잠시 걸릴 수 있습니다.</p></div>:tab==='friendly'?<Exhibition active={visible&&!savePanelOpen} roster={roster} loadRoster={client.loadRoster}/>:tab==='career'?<CareerPage active={visible&&!savePanelOpen} key={career.player?.id??'new'} player={career.player} teams={teams} leagueTeam={save?.team} busy={busy} edit={client.editCareer} onLeague={()=>openTab(save?'game':'league')}/>:!save?<>
    ...
    <section className="season-panel season-settings"><h2>새 리그 시작</h2><label>기록 시즌<select value={roster?.season??''} disabled={busy||yearBusy||!roster} onChange={e=>{setYearBusy(true);void client.loadRoster(Number(e.target.value)).catch(e=>client.setError(e.message)).finally(()=>setYearBusy(false));}}>{roster?.seasons.map(y=><option key={y} value={y}>{y} 정규시즌</option>)}</select></label><label>시즌 길이<select value={series} onChange={e=>setSeries(Number(e.target.value))}><option value={8}>정규 시즌 · 팀당 144경기</option><option value={4}>하프 시즌 · 팀당 72경기</option><option value={1}>미니 시즌 · 팀당 18경기</option></select></label><label>게임 속도<select value={pace} onChange={e=>setPace(e.target.value as Pace)}>{(Object.keys(PACE_SETTINGS) as Pace[]).map(p=><option key={p} value={p}>{PACE_SETTINGS[p].label}</option>)}</select></label><p className="season-note">3아웃 공수 교대 · 9이닝 · 연장 12회<br/>타격과 투구를 직접 조작하고, 원하는 구간은 자동 진행할 수 있습니다.</p><button className="season-primary" disabled={busy||yearBusy||!roster||!leagueReady} onClick={()=>send('create',{season:roster?.season,team,pace,seriesPerPair:series})}>리그 시작 <ArrowRight size={17}/></button>{roster&&!leagueReady&&<p className="season-note" role="status">이 시즌은 9명 타순과 투수를 갖춘 구단이 {playableTeams.length}개입니다. 정규 리그는 10개 구단의 기록이 있는 시즌을 선택해 주세요. 친선 경기는 두 구단으로도 진행할 수 있습니다.</p>}{!roster&&<button onClick={()=>void client.loadRoster().catch(e=>client.setError(e.message))}>선수 기록 다시 불러오기</button>}<button onClick={()=>openTab('career')}>먼저 내 선수 만들기</button></section></div>
   </>:tab==='game'?<>
    <div className="season-section-heading"><div><span>SEASON {save.seasonNumber} · DAY {Math.min(save.day,save.totalDays)} / {save.totalDays}</span><h1>{teamName(save.team)}의 시즌</h1><p>{played?.wins??0}승 {played?.losses??0}패 {played?.ties??0}무 · {save.season} 실제 기록 기반</p></div><button onClick={()=>openTab('league')}>순위와 일정 <ArrowRight size={16}/></button></div>
    <div className="season-game-layout"><div>{state?.action&&game?<SeasonMatch active={visible&&!savePanelOpen} key={game.id} state={state} busy={busy||savePanelOpen} clock={client.clock} command={command} appearances={appearances}/>:<section className="season-match-intro"><span>{save.complete?'SEASON COMPLETE':'NEXT GAME'}</span><h2>{save.complete?'시즌을 마쳤습니다':`${teamName(fixture?.awayTeam??'')} vs ${teamName(fixture?.homeTeam??'')}`}</h2><p>{save.complete?'순위와 개인 기록을 확인하고 다음 시즌에 도전하세요.':'타석과 마운드를 오가며 팀을 승리로 이끄세요.'}</p><button className="season-primary" disabled={busy} onClick={()=>save.complete?openTab('league'):send('start-game')}>{save.complete?'시즌 결과 보기':'경기 시작'} <Play size={18}/></button></section>}
    <section className="season-panel season-events"><h2>경기 기록</h2>{game?.events.length?<ol>{game.events.slice(-14).reverse().map((e,i)=><li key={game.events.length-i}>{e}</li>)}</ol>:<p className="season-note">경기가 시작되면 투구 결과와 득점 상황이 표시됩니다.</p>}</section></div>
    <aside><section className="season-panel"><h2>경기 진행</h2><div className="season-auto-actions">{career.player&&<button disabled={busy||save.complete||!!state?.action?.waiting} onClick={()=>send('sim-to-player')}>내 선수 차례까지</button>}<button disabled={busy||save.complete||!!game?.complete} onClick={()=>send('sim-half')}>이번 공수 자동 진행</button><button disabled={busy||save.complete||!!game?.complete} onClick={()=>send('sim-game')}>이번 경기 자동 진행</button><button disabled={busy||save.complete||!!game&&!game.complete} onClick={()=>send('sim-day')}>다음 일정 자동 진행</button></div><p className="season-note">우리 경기가 끝나면 같은 날 다른 구단의 경기 결과도 반영됩니다.</p></section>{teamRoster&&<TeamManagement save={save} busy={busy} pitchInFlight={!!state?.action?.pitch&&!state.action.pitch.resolved} send={send}/>}<section className="season-panel"><h2>마이 플레이어</h2>{career.player?<><p><b>{career.player.name}</b> · LV {career.player.level}</p><p className="season-note">훈련 {career.player.trainingPoints} PT · {career.player.games}경기 출전</p></>:<p className="season-note">나만의 선수를 만들어 구단 라인업에 합류하세요.</p>}<button onClick={()=>openTab('career')}>{career.player?'선수 육성':'선수 만들기'}</button></section></aside></div>
   </>:<>
    <div className="season-section-heading"><div><span>LEAGUE CENTER</span><h1>{save.seasonNumber}번째 시즌</h1><p>{save.season} 기록 기반 · 팀당 {save.totalDays}경기 · {save.complete?'시즌 종료':save.day+'일차'}</p></div><button className="season-primary" disabled={busy} onClick={()=>save.complete?send('next-season'):openTab('game')}>{save.complete?'다음 시즌 시작':'경기로 돌아가기'} <ArrowRight size={16}/></button></div>
