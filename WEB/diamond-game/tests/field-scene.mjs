...
for(const height of[155,185,215])for(const hand of[1,-1]){const scale=motion.pitcherBodyScale(height),model=createPlayer('#224466',true),mirror=new THREE.Group(),bat=new THREE.Group();mirror.add(model.root);mirror.scale.set(hand*scale,scale,scale);model.root.position.z=.06;
 // Use the actual skin wrist centre and returned off-axis glove cuff. The
 // physical barrel contact below remains exactly the existing .82 m point.
 for(const anticipation of[0,1])for(const x of[-1,0,1])for(const y of[-1,0,1])for(let time=0;time<=motion.SWING_DURATION_MS;time+=5){const localAim={x:x/scale,y:((1.05+y*.55)/scale-1.05)/.55},pose=motion.swingPose(time,localAim,hand,0,anticipation),wrists=poseBatter(model,mirror,bat,pose);for(const [arm,wrist]of[[model.le,wrists.leftWrist],[model.re,wrists.rightWrist]]){const error=arm.localToWorld(V(0,-.341,.006*playerDimensions(model.appearance).depthScale)).distanceTo(wrist);maxWrist=Math.max(maxWrist,error);assert(error<.01,`Custom batter hand detaches: ${height} ${hand} ${x} ${y} ${time} ${error}`);}if(time===95){const contact=bat.localToWorld(V(0,motion.BAT_SWEET_SPOT,0));assert(contact.distanceTo(V(x*.5,1.05+y*.55,0))<1e-10);}poses++;}
}
