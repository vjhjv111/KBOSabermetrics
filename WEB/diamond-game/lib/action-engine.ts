...
import {BodyHit,BodyCapsule,sweepBody,styleBodyCapsules} from "./pitch-feedback";
import {SWING_CONTACT_MS,normalizeDelivery,profileThrowingHand,pitchReleasePosition,pitcherBodyScale} from "./player-motion";
...
export interface ActionView {format:"action-v2";code:string;mode:"ai"|"pvp";role:Side;batter:string;pitcher:string;pace:Pace;round:number;balls:number;strikes:number;score:number;pitchCount:number;pitch:Pitch|null;history:PitchResult[];waiting:boolean;done:boolean;winner:Side|null;serverNow:number;serverReceivedAt?:number;serverSentAt?:number;expiresAt:number;roster?:MatchRoster;aiPitching?:{source:"observed"|"default";sampleCount:number;totalPitchCount:number;note?:string|null}}
...
export function throwsLeft(id:string){const p=playerProfile(id,"pitcher");return profileThrowingHand(p?.throws,p?.batsThrows)==="L"}
export function deliveryStyle(id:string){const p=playerProfile(id,"pitcher");return normalizeDelivery(p?.delivery,p?.batsThrows)}
export function isUnderhand(id:string){return deliveryStyle(id)==="underhand"}
export function isSidearm(id:string){return deliveryStyle(id)==="sidearm"}
export function pitcherScale(id:string){return pitcherBodyScale(playerProfile(id,"pitcher")?.heightCm)}
...
export function pitcherHandLabel(pid:string){const profile=playerProfile(pid,"pitcher");if(pid.includes(":")&&!profileThrowingHand(profile?.throws,profile?.batsThrows))return "투구손 정보 없음";return (throwsLeft(pid)?"좌투":"우투")+(isUnderhand(pid)?" · 언더핸드":isSidearm(pid)?" · 사이드암":"")}
...
export function incomingBallPosition(p:Pitch,result:PitchResult|undefined,time:number,side:Side){
 const base=(at:number)=>ballPosition(p,at);
...
export function findBodyHit(batter:string,pitcher:string,pitch:Pitch){
 const profile=playerProfile(batter,"batter"),scale=pitcherBodyScale(profile?.heightCm);
 const capsules:BodyCapsule[]=styleBodyCapsules(readyBody[batsLeft(batter,pitcher)?"left":"right"],scale,profile?.bodyType);
