...
function renderGame(d){
  const g=d.game,title=`${teamNames[g.Away]??g.Away} ${g.AScore??'—'} : ${g.HS??'—'} ${teamNames[g.Home]??g.Home}`;document.title=title+' · 경기 상세';const back=text('a','← 경기 달력','player-link');back.href='#games';gameRoot.replaceChildren(back,gameTeamScore(g,'h1'),text('p',`${g.Date?.slice(0,10)} · ${g.Stadium??''} · ${g.Status==='RESULT'?'경기 종료':g.Status??''}`));
