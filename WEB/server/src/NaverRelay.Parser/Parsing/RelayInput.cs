...
public static class RelayInput
{
    ...
    public static Input Read(string json)
    {
        ...
        if (root.TryGetProperty("naver", out var naver) || root.TryGetProperty("kboOfficial", out _))
        {
            ...
            // Mirrors UnifiedDocumentStore.RequiresKboOfficial: only the current season must be
            // cross-checked against KBO official data before a game counts as importable. A past
            // season that only ever collected Naver's own text relay (including backfilled/legacy
