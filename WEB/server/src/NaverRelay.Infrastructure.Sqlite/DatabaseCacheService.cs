...
public sealed partial class DatabaseCacheService : IWarehouseReadService
{
    ...
    public async Task<T?> TryLoadComputedAsync<T>(string cacheKey, CancellationToken cancellationToken = default)
    {
        ...
        if (WebReadOnly)
        {
            _webComputationSourceVersion.Value = sourceVersion;
            if (TryReadWebComputed(sourceVersion, cacheKey, out T? memoryValue)) return memoryValue;
        }
        await using var connection = await OpenAsync(cancellationToken).ConfigureAwait(false);
        await using var command = connection.CreateCommand();
    ...
    public async Task SaveComputedAsync<T>(string cacheKey, T value, CancellationToken cancellationToken = default)
    {
        if (WebReadOnly)
        {
            var expectedVersion = _webComputationSourceVersion.Value;
            _webComputationSourceVersion.Value = null;
            var currentVersion = await GetSourceVersionAsync(cancellationToken).ConfigureAwait(false);
            // Do not cache a result calculated while the collector committed a new DB version.
            if (expectedVersion is null || string.Equals(expectedVersion, currentVersion, StringComparison.Ordinal))
                SaveWebComputed(currentVersion, cacheKey, value);
            return;
        }
        var sourceVersion = await GetSourceVersionAsync(cancellationToken).ConfigureAwait(false);
        await using var connection = await OpenAsync(cancellationToken).ConfigureAwait(false);
