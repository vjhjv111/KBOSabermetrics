...
public sealed partial class DatabaseAnalyticsService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (!pitcher) await AttachTeamBattingContextAsync(query, cached, cancellationToken).ConfigureAwait(false);
            return cached;
        }
        var data = await _database.GetWebRoleAggregateDataAsync(query, pitcher, cancellationToken).ConfigureAwait(false);
        if (!pitcher) await AttachTeamBattingContextAsync(query, data, cancellationToken).ConfigureAwait(false);
