    ...
    public async Task<TablePage> QueryAsync(RecordRequest request, CancellationToken token)
    {
        ...
        if (request.Role == "batter" && request.View == "team-batting")
            warnings.Add(request.Room == "team"
                ? "병살 상황은 2아웃 미만에 1루 주자가 있고 타석 결과 전까지 1루를 떠나지 않은 타석입니다. 팀 잔루는 PA-득점-아웃, 희생번트 실패는 주자가 있는 2아웃 미만의 번트 관련 타석 중 안타·희생타 성공·실책 출루·볼넷류가 아닌 타자 아웃입니다. 번트 파울·번트 헛스윙 뒤 일반 플라이·땅볼 등으로 아웃된 타석도 포함하며, 야수선택은 제외합니다."
                : "병살 상황은 2아웃 미만에 1루 주자가 있고 타석 결과 전까지 1루를 떠나지 않은 타석입니다. 선수 잔루는 타자가 아웃된 플레이 후 남은 주자 수, 희생번트 실패는 주자가 있는 2아웃 미만의 번트 관련 타석 중 안타·희생타 성공·실책 출루·볼넷류가 아닌 타자 아웃입니다. 번트 파울·번트 헛스윙 뒤 일반 플라이·땅볼 등으로 아웃된 타석도 포함하며, 야수선택은 제외합니다.");
