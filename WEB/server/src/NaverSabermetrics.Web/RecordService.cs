    ...
    public async Task<TablePage> QueryAsync(RecordRequest request, CancellationToken token)
    {
        ...
        if (request.Role == "batter" && request.View == "team-batting")
            warnings.Add(request.Room == "team"
                ? "병살 상황은 2아웃 미만에 1루 주자가 있고 타석 결과 전까지 1루를 떠나지 않은 타석입니다. 팀 잔루는 PA-득점-아웃, 희생번트 실패는 번트 타구·번트 파울 후 아웃 또는 야수선택·번트 헛스윙이 확인된 타석 중 타석 시작 시 가장 앞선 주자가 다음 베이스나 홈까지 진루하지 못한 경우입니다. 번트 파울 뒤 일반 타격 아웃이나 선행주자 포스아웃도 포함하며, 안타·볼넷·몸에 맞는 볼 출루는 제외하고 실책에 의한 진루는 실패로 계산합니다."
                : "병살 상황은 2아웃 미만에 1루 주자가 있고 타석 결과 전까지 1루를 떠나지 않은 타석입니다. 선수 잔루는 타자가 아웃된 플레이 후 남은 주자 수, 희생번트 실패는 번트 타구·번트 파울 후 아웃 또는 야수선택·번트 헛스윙이 확인된 타석 중 타석 시작 시 가장 앞선 주자가 다음 베이스나 홈까지 진루하지 못한 경우입니다. 번트 파울 뒤 일반 타격 아웃이나 선행주자 포스아웃도 포함하며, 안타·볼넷·몸에 맞는 볼 출루는 제외하고 실책에 의한 진루는 실패로 계산합니다.");
