...
public sealed partial class RecordService
{
    ...
    public const string FormulaVersion = "Uploaded-KboPitcherWarV3-web.4";
    ...
    public async Task<TablePage> QueryAsync(RecordRequest request, CancellationToken token)
    {
        ...
        var key = $"web-v3-result-v3|{dataVersion}|{definition.Role}|{definition.Key}|{JsonSerializer.Serialize(query)}|{request.Position}|{request.QualificationPercent.ToString(CultureInfo.InvariantCulture)}";
        ...
        if (request.Role == "batter" && request.View == "team-batting")
            warnings.Add(request.Room == "team"
                ? "병살 상황은 2아웃 미만에 1루 주자가 있고 타석 결과 전까지 1루를 떠나지 않은 타석입니다. 팀 잔루는 PA-득점-아웃, 희생번트 실패는 네이버 중계 원시 이벤트에서 주자 존재·2아웃 미만의 희생/안타가 아닌 번트 타구 또는 번트 파울 후 삼진으로 재구성합니다."
                : "병살 상황은 2아웃 미만에 1루 주자가 있고 타석 결과 전까지 1루를 떠나지 않은 타석입니다. 선수 잔루는 타자가 아웃된 플레이 후 남은 주자 수, 희생번트 실패는 네이버 중계 원시 이벤트에서 주자 존재·2아웃 미만의 희생/안타가 아닌 번트 타구 또는 번트 파울 후 삼진으로 재구성합니다.");
