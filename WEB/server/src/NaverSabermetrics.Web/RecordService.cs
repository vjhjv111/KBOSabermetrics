    ...
    public async Task<TablePage> QueryAsync(RecordRequest request, CancellationToken token)
    {
        ...
        if (request.Role == "batter" && request.View == "team-batting")
            warnings.Add(request.Room == "team"
                ? "병살 상황은 2아웃 미만에 1루 주자가 있고 타석 결과 전까지 1루를 떠나지 않은 타석입니다. 팀 잔루는 PA-득점-아웃, 희생번트 실패는 번트 타구·번트 파울 후 아웃 또는 야수선택·번트 헛스윙이 확인된 타석을 기준으로 계산합니다. 실제 번트 타구는 타석 시작 시 가장 앞선 주자가 다음 베이스나 홈까지 진루하지 못하면 실패이며, 번트 시도 뒤 일반 플라이·땅볼 등으로 전환해 아웃된 타석은 주자 진루 여부와 관계없이 실패에 포함합니다. 안타·볼넷·몸에 맞는 볼 출루는 제외하고 실책에 의한 진루는 실패로 계산합니다."
                : "병살 상황은 2아웃 미만에 1루 주자가 있고 타석 결과 전까지 1루를 떠나지 않은 타석입니다. 선수 잔루는 타자가 아웃된 플레이 후 남은 주자 수, 희생번트 실패는 번트 타구·번트 파울 후 아웃 또는 야수선택·번트 헛스윙이 확인된 타석을 기준으로 계산합니다. 실제 번트 타구는 타석 시작 시 가장 앞선 주자가 다음 베이스나 홈까지 진루하지 못하면 실패이며, 번트 시도 뒤 일반 플라이·땅볼 등으로 전환해 아웃된 타석은 주자 진루 여부와 관계없이 실패에 포함합니다. 안타·볼넷·몸에 맞는 볼 출루는 제외하고 실책에 의한 진루는 실패로 계산합니다.");
