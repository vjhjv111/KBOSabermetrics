...
서버는 한국시간 날짜별로 웹 진입 문서(`/`, `/index.html`) 요청을 집계합니다. 원 IP는 저장하지 않고 서버 키로 만든 해시만 당일 중복 제거에 사용하며, 이미지·스크립트·API 요청은 페이지 방문 수에서 제외합니다. Render 영구 디스크의 다음 파일에서 날짜별 고유 방문자와 페이지 진입 횟수를 확인할 수 있습니다.

```bash
column -t -s $'\t' /var/data/state/analytics/daily-visitors.tsv
```
